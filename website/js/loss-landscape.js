...
class LossLandscapeViz {
    ...
    computeDotLoss(w1, w2) {
        ...
        for (const sample of xorData) {
            const dot = w1 * sample.x[0] + w2 * sample.x[1];
            const pred = 1 / (1 + Math.exp(-dot));
            const clipped = Math.max(1e-7, Math.min(1 - 1e-7, pred));
            totalLoss += -(sample.y * Math.log(clipped) + (1 - sample.y) * Math.log(1 - clipped));
        }
        return totalLoss / 4;
    }
    ...
    computeYatLoss(w1, w2) {
        ...
        for (const sample of xorData) {
            const dot = w1 * sample.x[0] + w2 * sample.x[1];
            const dx = w1 - sample.x[0];
            const dy = w2 - sample.x[1];
            const sqDist = dx * dx + dy * dy;
            const yat = (dot * dot) / (sqDist + epsilon);
            
            // Soft threshold for YAT
            const pred = 1 / (1 + Math.exp(-(yat - 0.3) * 5));
            const clipped = Math.max(1e-7, Math.min(1 - 1e-7, pred));
            totalLoss += -(sample.y * Math.log(clipped) + (1 - sample.y) * Math.log(1 - clipped));
        }
        return totalLoss / 4;
