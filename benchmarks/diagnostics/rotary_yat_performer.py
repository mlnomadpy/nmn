...
def test_normalize_qk():
    ...
    print(
        f"  K norms after normalization (should be ~1): mean={float(jnp.mean(k_norms)):.6f}, std={float(jnp.std(k_norms)):.6g}"
    )
...
def test_rotary_yat_performer():
    ...
    projection = create_yat_tp_projection(k4, head_dim=head_dim, num_prf_features=8)
    freqs_cos, freqs_sin = precompute_freqs_cis(head_dim, max_seq_len)
...
def test_gradient_flow():
    ...
    target = jax.random.normal(
        jax.random.split(key)[0], (batch_size, seq_len, embed_dim)
    )
...
def test_feature_map_kernel_approximation():
    ...
    print(
        f"Relative error: {abs(approx_kernel - exact_yat) / abs(exact_yat) * 100:.2f}%"
    )
