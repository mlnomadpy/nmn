...
def benchmark_fn(
    fn: Callable, *args, warmup: int = WARMUP_ITERS, iters: int = BENCHMARK_ITERS
) -> Tuple[float, float]:
...
def print_benchmark_header():
    ...
    print("=" * 80)
    print(
        f"Config: batch={BATCH_SIZE}, heads={NUM_HEADS}, head_dim={HEAD_DIM}, embed_dim={EMBED_DIM}"
    )
    ...
    for seq_len in SEQ_LENGTHS:
        ...
        try:
            results["Dot-Product (baseline)"][
                seq_len
            ] = bench.benchmark_dot_product_attention()
            print(
                f"  Dot-Product: {format_time(*results['Dot-Product (baseline)'][seq_len])}"
            )
        ...
        try:
            results["YAT Normalized"][
                seq_len
            ] = bench.benchmark_yat_attention_normalized()
            print(
                f"  YAT Normalized: {format_time(*results['YAT Normalized'][seq_len])}"
            )
        except Exception as e:
        ...
        try:
            results["YAT Performer + Normalize"][
                seq_len
            ] = bench.benchmark_yat_performer_normalized()
            print(
                f"  YAT Performer + Norm: {format_time(*results['YAT Performer + Normalize'][seq_len])}"
            )
        except Exception as e:
        ...
        try:
            ...
            print(
                f"  Rotary YAT: {format_time(*results['Rotary YAT Standard'][seq_len])}"
            )
        except Exception as e:
            print(f"  Rotary YAT: FAILED ({e})")

        try:
            results["Rotary YAT Performer"][
                seq_len
            ] = bench.benchmark_rotary_yat_performer()
            print(
                f"  Rotary YAT Performer: {format_time(*results['Rotary YAT Performer'][seq_len])}"
            )
        except Exception as e:
            print(f"  Rotary YAT Performer: FAILED ({e})")

        try:
            results["Rotary YAT Performer + Normalize"][
                seq_len
            ] = bench.benchmark_rotary_yat_performer_normalized()
            print(
                f"  Rotary YAT Perf+Norm: {format_time(*results['Rotary YAT Performer + Normalize'][seq_len])}"
            )
        except Exception as e:
...
class TestAttentionSpeed:
    ...
    def test_performer_scales_linearly(self, benchmark_results):
        ...
        print(
            f"{'Transition':>15} | {'Seq Ratio':>10} | {'Perf Ratio':>12} | {'Status':>10}"
        )
        print("-" * 55)
        ...
        for i in range(1, len(seq_lens)):
            ...
            print(
                f"{seq_lens[i-1]:>6} -> {seq_lens[i]:<5} | {ratio_seq:>10.1f}x | {ratio_time:>12.2f}x | {status:>10}"
            )
...
class TestAttentionCorrectness:
    ...
    def test_all_attention_outputs_valid(self):
        ...
        tests = [
            ...
            (
                "YAT Performer + Norm",
                lambda: yat_performer_attention(
...
class TestMemoryScaling:
    ...
    def test_memory_analysis(self):
        ...
        print()
        print(
            f"{'Seq Len':>10} | {'Quadratic (n²)':>15} | {'Linear (n)':>15} | {'Ratio':>10}"
        )
        print("-" * 60)
        ...
        for seq_len in [256, 512, 1024, 2048, 4096, 8192]:
            ...
            print(
                f"{seq_len:>10} | {quadratic_mb:>12.2f} MB | {linear_mb:>12.2f} MB | {ratio:>8.1f}x"
            )
...
class TestScalingAnalysis:
    ...
    def test_scaling_analysis(self):
        ...
        print(
            f"{'Seq Len Change':>20} | {'YAT (expect 4x)':>15} | {'Performer (expect 2x)':>20}"
        )
        print("-" * 60)
        ...
        for i in range(1, len(seq_lengths)):
            ...
            perf_ratio = (
                perf_times[i] / perf_times[i - 1] if perf_times[i - 1] > 0 else 0
            )
