...
def test_tf_yat_nmn_constant_alpha():
    ...
    x_np = np.random.randn(2, 4).astype(np.float32)
    layer = YatNMN(
        features=3, use_bias=False, use_alpha=False, constant_alpha=True, epsilon=1e-5
    )
...
def test_tf_yat_nmn_get_set_weights_with_constant_alpha():
    ...
    assert all(w is not None for w in weights), "get_weights() returned None entries"
