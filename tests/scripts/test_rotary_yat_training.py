...
def print_stats(name: str, arr: jnp.ndarray):
    ...
    if arr is None:
        ...
        return
    print(
        f"{name}: min={float(jnp.min(arr)):.4g}, max={float(jnp.max(arr)):.4g}, "
...
def test_gradient_magnitudes():
    ...
    target = jax.random.normal(
        jax.random.split(key)[0], (batch_size, maxlen, embed_dim)
    )
        ...
        if hasattr(grads, name):
            ...
            if hasattr(proj, "kernel") and hasattr(proj.kernel, "value"):
                ...
                if g is not None:
                    ...
                    print(
                        f"  {status} {name}: max_abs={max_grad:.6g}, mean_abs={mean_grad:.6g}, nan={has_nan}, inf={has_inf}"
                    )
...
def test_feature_map_behavior():
    ...
    for angle_factor in [0.0, 0.25, 0.5, 0.75, 1.0]:
        ...
        print(
            f"  angle_factor={angle_factor:.2f}, dot={dot:+.4f}, exact_yat={exact_yat:.4f}, approx={approx:.4f}"
        )
...
def test_long_sequence_scaling():
    ...
    for seq_len in [16, 64, 256, 512, 1024]:
        ...
        print(
            f"  seq_len={seq_len:4d}: output_std={out_std:.4f}, ratio={out_std/in_std:.4f}"
        )
