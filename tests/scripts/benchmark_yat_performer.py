...
def benchmark_approximation_quality():
    ...
    for seq_len in [32, 64, 128, 256, 512]:
        ...
        params = create_yat_tp_projection(
            k4, head_dim, num_prf_features=64, num_quad_nodes=8
        )

        # Normalize inputs for performer too
        q_p, k_p = normalize_qk(q, k)
        approx_out = yat_tp_attention(
            q_p, k_p, v, params, causal=False, gradient_scaling=True
        )
        ...
        if norm_exact > 0 and norm_approx > 0:
            cosine_sim = float(
                jnp.sum(exact_flat * approx_flat) / (norm_exact * norm_approx)
            )
        ...
        print(
            f"{seq_len:>8} | {cosine_sim:>12.4f} | {l2_error:>12.4f} | {max_diff:>12.4f}"
        )
...
def benchmark_inference_speed():
    ...
    for seq_len in [128, 256, 512, 1024]:
        ...
        # JIT compile
        exact_fn = jax.jit(
            lambda q, k, v: yat_attention(*normalize_qk(q, k), v, epsilon=1e-5)
        )
        perf_fn = jax.jit(
            lambda q, k, v: yat_tp_attention(
                *normalize_qk(q, k), v, params, causal=False
            )
        )
        ...
        print(
            f"{seq_len:>8} | {exact_time:>12.2f} | {perf_time:>12.2f} | {speedup:>10.2f}x"
        )
...
def benchmark_gradient_flow_depth():
    ...
    for num_layers in [1, 2, 4, 8]:
        ...
        print(
            f"{num_layers:>8} | {input_grad:>12.2e} | {out_mag:>12.2f} | {ratio:>10.2f}"
        )
