...
class TestYatNMNMathValidation:
    ...
    def test_yat_formula_with_alpha_scaling(self):
        ...
        with torch.no_grad():
            ...
            np.testing.assert_allclose(
                out_with_alpha.numpy(), expected.numpy(), rtol=1e-4
            )
    ...
    def test_epsilon_prevents_division_by_zero(self):
        ...
        layer = YatNMN(
            in_features=4, out_features=2, bias=False, alpha=False, epsilon=1e-6
        )
...
class TestYatConv2DMathValidation:
    ...
    def test_yat_conv2d_distance_computation(self):
        ...
        layer = YatConv2D(
            in_channels=1, out_channels=1, kernel_size=2, bias=False, use_alpha=False
        )
    ...
    def test_yat_conv2d_non_matching_patch_kernel(self):
        ...
        with torch.no_grad():
            ...
            # Manual computation
            patch = np.array([4.0, 3.0, 2.0, 1.0])
...
class TestYatConvTranspose2DMathValidation:
    ...
    def test_yat_conv_transpose2d_positive_outputs(self):
        ...
        with torch.no_grad():
            ...
            assert (
                output >= 0
            ).all(), "Transposed conv should produce non-negative values"
