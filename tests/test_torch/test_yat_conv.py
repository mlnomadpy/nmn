...
def test_yat_conv1d_forward():
    ...
    try:
        ...
        layer = YatConv1D(in_channels=16, out_channels=32, kernel_size=3, padding=1)
...
def test_yat_conv3d_forward():
    ...
    try:
        ...
        layer = YatConv3D(in_channels=8, out_channels=16, kernel_size=3, padding=1)
...
def test_yat_conv2d_with_alpha():
    ...
    try:
        ...
        layer = YatConv2D(in_channels=3, out_channels=16, kernel_size=3, use_alpha=True)
...
def test_yat_conv_all_variants_with_device():
    ...
    try:
        ...
        # Test YatConv1D
        layer1d = YatConv1D(in_channels=8, out_channels=16, kernel_size=3, device="cpu")
        assert layer1d is not None
        assert layer1d.weight.device.type == "cpu"

        # Test YatConv2D
        layer2d = YatConv2D(in_channels=3, out_channels=16, kernel_size=3, device="cpu")
        assert layer2d is not None
        assert layer2d.weight.device.type == "cpu"

        # Test YatConv3D
        layer3d = YatConv3D(in_channels=8, out_channels=16, kernel_size=3, device="cpu")
...
def test_yat_conv2d_dtype_parameter():
    ...
    try:
        ...
        layer_f32 = YatConv2D(
            in_channels=3, out_channels=16, kernel_size=3, dtype=torch.float32
        )
        ...
        layer_f64 = YatConv2D(
            in_channels=3, out_channels=16, kernel_size=3, dtype=torch.float64
        )
