...
def _dense_value_and_grads(dtype):
    layer = YatNMN(
        2, 1, bias=False, alpha=False, epsilon=1.0, dtype=dtype, param_dtype=dtype
    )
    with torch.no_grad():
...
def _aggregate_dense_grads(dtype):
    layer = YatNMN(
        2, 1, bias=True, alpha=True, epsilon=1.0, dtype=dtype, param_dtype=dtype
    )
    with torch.no_grad():
        layer.weight.copy_(torch.tensor([[-100.0, -99.0]], dtype=dtype))
...
def test_fp16_dense_aggregate_cotangents_match_saturated_fp32_reference():
    ...
    np.testing.assert_allclose(
        output.float().detach().numpy(),
        reference_output.detach().numpy(),
        rtol=5e-3,
        atol=2.0,
    )
...
def test_low_precision_dense_and_attention_preserve_genuine_nan():
    ...
    assert torch.isnan(
        layer(torch.tensor([[float("nan"), 1.0]], dtype=torch.float16))
    ).all()
    ...
    weights = yat_attention(
        query,
        key,
        torch.ones((1, 2, 1, 1), dtype=torch.float16),
        training=False,
    )
