...
class TestYatAttentionAlpha:
    ...
    def test_yat_attention_normalized_with_alpha(self):
        ...
        output_with_alpha = yat_attention_normalized(
            query, key_arr, value, alpha=jnp.sqrt(2.0)
        )
    ...
    def test_yat_performer_attention_with_alpha(self):
        ...
        # Without alpha
        output_no_alpha = yat_performer_attention(
            query, key_arr, value, projection, alpha=None
        )

        # With alpha
        output_with_alpha = yat_performer_attention(
            query, key_arr, value, projection, alpha=1.5
        )
...
class TestMultiHeadAttentionAlpha:
    ...
    def test_multihead_attention_constant_alpha_true(self):
        ...
        from nmn.nnx.layers.attention import DEFAULT_CONSTANT_ALPHA, MultiHeadAttention
...
class TestAlphaScalingFormula:
    ...
    def test_scaling_formula_values(self):
        ...
        for head_dim in head_dims:
            ...
            print(
                f"  {head_dim:>10} | {float(scale_1):>12.4f} | {float(scale_sqrt2):>14.4f}"
            )
