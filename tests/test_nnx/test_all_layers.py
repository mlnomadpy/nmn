...
class TestLayerImports:
    ...
    def test_import_yat_attention(self):
        ...
        from nmn.nnx.layers.attention import MultiHeadAttention, yat_attention_weights
...
class TestYatNMNComprehensive:
    ...
    def test_positive_outputs_no_bias(self):
        ...
        layer = YatNMN(
            in_features=8, out_features=16, use_bias=False, use_alpha=False, rngs=rngs
        )
...
class TestYatConvComprehensive:
    ...
    def test_conv1d_forward_valid_padding(self):
        ...
        rngs = nnx.Rngs(0)
        layer = YatConv(
            in_features=3, out_features=8, kernel_size=(3,), padding="VALID", rngs=rngs
        )
        x = jnp.ones((2, 16, 3))
        output = layer(x)
    ...
    def test_conv1d_forward_same_padding(self):
        ...
        rngs = nnx.Rngs(0)
        layer = YatConv(
            in_features=3, out_features=8, kernel_size=(3,), padding="SAME", rngs=rngs
        )
        x = jnp.ones((2, 16, 3))
        output = layer(x)
    ...
    def test_conv2d_forward_same_padding(self):
        ...
        rngs = nnx.Rngs(0)
        layer = YatConv(
            in_features=3, out_features=8, kernel_size=(3, 3), padding="SAME", rngs=rngs
        )
        x = jnp.ones((2, 16, 16, 3))
        output = layer(x)
...
class TestMultiHeadAttention:
    ...
    def test_attention_forward(self):
        ...
        attn = MultiHeadAttention(num_heads=4, in_features=32, rngs=rngs)
