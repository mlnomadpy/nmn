...
| KG03 stable observability; kernel sensors; LEACE | Raw activations, sensor outputs, bank spectra, labels, covariance/noise/separation data | Kernel/sensor values, spectra, Jacobians, covariance, sampled separation, supplied noise-radius diagnostics, fit-only linear/quadratic state probes with frozen and separately refitted held-out edit comparisons | Broader nonlinear probe families, sensor acquisition, stochastic-noise protocols, stable inverse bounds |
| KG04 interaction structure; Integrated Hessians; HVP patching correction | Joint effects, background-dependent finite differences, gradients, mixed curvature, path and quadrature data | Budgeted module-subset lattices on native graphs, explicit background gates, full-lattice subset coefficients, gate derivatives, directional Hessian-vector products and joint directional path integration | Vectorized HVP batching, sparse recovery and certified residuals |
| KG05 native protected edits; RAVEL; ACDC/EAP-IG | Native edit and donor coordinates, recomputed descendants, per-example target/protection outputs, eligibility | Native gates/replacements, receiving-module read-slot patches, producer-specific residual edge studies and declared donor-edge transfers with CLI/replay/export, donor studies with provenance, full traces and effects, declared classification tasks, per-example damage and semantic strata, bounded gate proposals with measured protection selection | Model-based donor acquisition, recursive path isolation, broader native feasibility solvers, source-faithful comparator adapters |
| KG06 acquisition/selection; Learn then Test | Population IDs, independent splits, per-candidate risk/uncertainty, selection/stopping history, full cost | Sample IDs, split/group checks, explicit donor access, budgeted metadata-matched pair plans, executable budgeted candidate selection, frozen-winner validation, ledger, controls and per-example observations | Statistical validation and acquisition algorithms |
| KG07 semantics; causal abstraction/scrubbing; patching illusion | Semantic variables/reference outputs, donor equivalence groups, correspondence hypotheses, predicted/unforced outcomes | Supplied semantic dataset records, finite reference/correspondence tables, versioned reference callback, native donor transfer, bounded supervised assignment selection and counterfactual mismatch reports | Recursive scrubbing, distributed semantic alignment and ambiguity controls |
| Kernelized erasure / preimage reconstruction | Feature projections, realizable native edits, residual feature and nonlinear suffix error | Kernel bank features, bounded per-example native preimage search and executed suffix responses | Learned projection/mapper, source-faithful erasure algorithm and utility/probe comparators |
| NAM / IMQ / generic smooth comparison | Matched task, topology, supervision, fit budget, effects and serving costs | Native IMQ/tanh/linear alternatives, matched synthetic fits, explicit donor-gradient policy, CPU replay and parameter/precision/cost records | Broader tasks, independent initialization studies and acquisition/certification baselines |
| Formal verification literature (AutoBound, Beta-CROWN, certified fidelity) | Domain, arithmetic assurance, sound bounds, counterexamples and unresolved results | Exact rational enclosures, bounded output/output-difference verification with structural zero dependencies and search-independent partition checking for fixed Yat/IMQ/linear graphs | Distinct-model relations, additional operations and runtime-roundoff bounds |
...
## Discover runnable workflows

`nmn research workflows` returns a backend-free JSON catalog of command groups,
required inputs, raw outputs, backend scope and remaining obligations. Filter with
`--family KG04` (KG01–KG07). Use each command’s `--help` for exact arguments.

[Semantic assignment search](native-alignment.md), [directional curvature](native-curvature.md),
