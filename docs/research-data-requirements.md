...
| KG05 native protected edits; RAVEL; ACDC/EAP-IG | Native edit and donor coordinates, recomputed descendants, per-example target/protection outputs, eligibility | Native gates/replacements, receiving-module read-slot patches, producer-specific residual read patches in the graph API, donor studies with provenance, full traces and effects, declared classification tasks, per-example damage and semantic strata | Edge-patch study/CLI/replay integration, recursive path isolation, native feasibility search, source-faithful comparator adapters |
