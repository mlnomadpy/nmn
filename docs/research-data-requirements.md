...
| KG06 acquisition/selection; Learn then Test | Population IDs, independent splits, per-candidate risk/uncertainty, selection/stopping history, full cost | Sample IDs, metadata, controls and per-example observations | Enforced split/pair discipline, candidate ledger, statistical validation and acquisition algorithms |
| KG07 semantics; causal abstraction/scrubbing; patching illusion | Semantic variables/reference outputs, donor equivalence groups, correspondence hypotheses, predicted/unforced outcomes | User metadata and named native states only | Typed semantic dataset schema, reference-model interface, interchange/scrubbing protocols and ambiguity controls |
...
| NAM / IMQ / generic smooth comparison | Matched task, topology, supervision, fit budget, effects and serving costs | Reusable measurements for the native reference | Matched alternative modules, benchmark runner and training lifecycle |
