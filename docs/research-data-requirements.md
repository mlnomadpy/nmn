...
| KG05 native protected edits; RAVEL; ACDC/EAP-IG | Native edit and donor coordinates, recomputed descendants, per-example target/protection outputs, eligibility | Native gates/replacements, receiving-module read-slot patches, producer-specific residual edge studies with CLI/replay/export, donor studies with provenance, full traces and effects, declared classification tasks, per-example damage and semantic strata | Donor-edge selection protocols, recursive path isolation, native feasibility search, source-faithful comparator adapters |
