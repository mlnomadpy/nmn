...
def example_1_basic_yat_layers():
    ...
    # YAT Linear layer
    fc = YatNMN(in_features=256, out_features=128, alpha=True, epsilon=1e-5)
...
def example_2_weight_normalization():
    ...
    print(
        f"✓ Conv filter norms: min={filter_norms.min():.4f}, max={filter_norms.max():.4f}"
    )

    fc_weight = fc_normalized.weight.data
    neuron_norms = torch.sqrt(torch.sum(fc_weight**2, dim=1))
    print(
        f"✓ Linear neuron norms: min={neuron_norms.min():.4f}, max={neuron_norms.max():.4f}"
    )
...
def example_3_weight_tying():
    ...
    layer1 = YatConv2D(
        3, 32, 3, padding=1, tie_kernel_bank=True, kernel_bank_id="shared-conv"
    )
    print(f"Layer 1: 32 filters, weight shape: {layer1.weight.shape}")

    layer2 = YatConv2D(
        32, 64, 3, padding=1, tie_kernel_bank=True, kernel_bank_id="shared-conv"
    )
    print(f"Layer 2: 64 filters (auto-expanded), weight shape: {layer2.weight.shape}")

    layer3 = YatConv2D(
        64, 128, 3, padding=1, tie_kernel_bank=True, kernel_bank_id="shared-conv"
    )
...
def example_4_constant_alpha():
    ...
    print(
        f"✓ Conv with learnable alpha: has parameter 'alpha' = {conv_learnable.alpha is not None}"
    )

    # Constant alpha = sqrt(2)
    conv_constant_sqrt2 = YatConv2D(3, 64, 3, padding=1, constant_alpha=True)
    print(
        f"✓ Conv with constant alpha=√2 ({conv_constant_sqrt2._constant_alpha_value:.4f})"
    )

    # Custom constant alpha
    conv_constant_custom = YatConv2D(3, 64, 3, padding=1, constant_alpha=2.0)
    print(
        f"✓ Conv with constant alpha=2.0 ({conv_constant_custom._constant_alpha_value})"
    )
