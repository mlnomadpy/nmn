...
def _yat_scores(
    ...
    """
    dot = jnp.einsum("...qhd,...khd->...hqk", q_f32, k_f32, precision=precision)
...
def _fused_yat_l1_attn(
    ...
    scores, L, _, _, _, _ = _yat_scores(
        q_f32,
        k_f32,
        bias.astype(jnp.float32) if has_bias else bias,
        epsilon.astype(jnp.float32),
        mask,
        has_bias,
        has_mask,
        scale,
...
def _fused_yat_l1_attn_bwd(has_bias, has_mask, has_eps_grad, scale, precision, res, g):
    ...
    if has_eps_grad:
        g_eps = _unbroadcast_to(dS * (-(num**2) * inv_dist_scale_sq), epsilon)
    else:
...
def _yat_self_scores(x_f32, bias, epsilon, has_bias, scale, precision):
    ...
    dot = jnp.einsum("...qhd,...khd->...hqk", x_f32, x_f32, precision=precision)
...
def _fused_yat_l1_self_attn(
    x, value, bias, epsilon, has_bias, has_eps_grad, scale, precision
):
...
def _fused_yat_l1_self_attn_fwd(
    x, value, bias, epsilon, has_bias, has_eps_grad, scale, precision
):
    x_f32 = x.astype(jnp.float32)
...
def _fused_yat_l1_self_attn_bwd(has_bias, has_eps_grad, scale, precision, res, g):
    ...
    dx_from_dot_q = jnp.einsum(
        "...hqk,...khd->...qhd", g_dot_total, x_f32, precision=precision
    )
    dx_from_dot_k = jnp.einsum(
        "...hqk,...qhd->...khd", g_dot_total, x_f32, precision=precision
    )
